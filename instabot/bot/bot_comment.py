...
def comment(self, media_id, comment_text):
    ...
    if not self.reached_limit('comments'):
        ...
        self.console_print("Out of comments for today.")
    return False


def reply_to_comment(self, media_id, comment_text, parent_comment_id):
    if not self.is_commented(media_id):
        self.console_print("Media is not commented yet, nothing to answer to...")
        return False
    if not self.reached_limit('comments'):
        if self.blocked_actions['comments']:
            self.logger.warning('YOUR `COMMENT` ACTION IS BLOCKED')
        ...
        if _r == 'feedback_required':
            ...
            return False
        if _r:
            self.console_print('Replied to comment {} of media {}'.format(parent_comment_id, media_id))
            self.total['comments'] += 1
            return True
    else:
        self.console_print("Out of comments for today.")
...
def comment_medias(self, medias):
    broken_items = []
    self.console_print("Going to comment %d medias." % (len(medias)))
    for media in tqdm(medias):
        if not self.is_commented(media):
            text = self.get_comment()
            self.console_print("Commented with text: %s" % text)
            if not self.comment(media, text):
                self.delay('comment')
                broken_items = medias[medias.index(media):]
                break
    self.console_print("DONE: Total commented on %d medias. " %
                     self.total['comments'])
    return broken_items


def comment_hashtag(self, hashtag, amount=None):
    self.console_print("Going to comment medias by %s hashtag" % hashtag)
...
def comment_user(self, user_id, amount=None):
    ...
    if not self.check_user(user_id, filter_closed_acc=True):
        return False
    self.console_print("Going to comment user_%s's feed:" % user_id)
