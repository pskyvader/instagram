...
def upload_photo(self, photo, caption=None, upload_id=None, from_video=False):
    ...
    if self.api.upload_photo(photo, caption, upload_id, from_video):
        self.console_print("Photo '{}' is uploaded.".format(photo))
        return True
    self.console_print("Photo '{}' is not uploaded.".format(photo))
    return False
...
def download_photo(self, media_id, folder='photos', filename=None, save_description=False):
    ...
    try:
        ...
        self.console_print("Media with `{}` is not downloaded.".format(media_id))
        return False
...
def download_photos(self, medias, folder, save_description=False):
    broken_items = []
    if not medias:
        self.console_print("Nothing to downloads.")
        return broken_items
    self.console_print("Going to download {} medias.".format(len(medias)))
