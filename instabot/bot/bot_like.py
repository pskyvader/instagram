...
def like(self, media_id, check_media=True):
    if not self.reached_limit('likes'):
        ...
        if _r == 'feedback_required':
            ...
            return False
        if _r:
            self.console_print("Liked media %d." % media_id)
            self.total['likes'] += 1
            return True
    else:
        self.console_print("Out of likes for today.")
    return False
...
def like_comment(self, comment_id):
    if not self.reached_limit('likes'):
        ...
        if _r == 'feedback_required':
            ...
            return False
        if _r:
            self.console_print("Liked comment {}.".format(comment_id))
            self.total['likes'] += 1
            return True
    else:
        self.console_print("Out of likes for today.")
...
def like_media_comments(self, media_id):
    ...
    self.console_print('Found {} comments'.format(len(media_comments)))
    comment_ids = [item["pk"] for item in media_comments if not item.get('has_liked_comment') or not item["has_liked_comment"]]

    if not comment_ids:
        self.console_print("None comments received: comments not found or comments have been filtered.")
        return broken_items

    self.console_print("Going to like %d comments." % (len(comment_ids)))

    for comment in tqdm(comment_ids):
        if not self.like_comment(comment):
            self.error_delay()
            broken_items = comment_ids[comment_ids.index(comment):]
    self.console_print("DONE: Liked {count} comments.".format(
...
def like_medias(self, medias, check_media=True):
    broken_items = []
    if not medias:
        self.console_print("Nothing to like.")
        return broken_items
    self.console_print("Going to like %d medias." % (len(medias)))
    for media in tqdm(medias):
        if not self.like(media, check_media):
            self.error_delay()
            broken_items.append(media)
    self.console_print("DONE: Total liked %d medias." % self.total['likes'])
    return broken_items


def like_timeline(self, amount=None):
    self.console_print("Liking timeline feed:")
...
def like_user(self, user_id, amount=None, filtration=True):
    ...
    if filtration:
        if not self.check_user(user_id):
            return False
    self.console_print("Liking user_%s's feed:" % user_id)
...
def like_hashtag(self, hashtag, amount=None):
    ...
    self.console_print("Going to like media with hashtag #%s." % hashtag)
...
def like_followers(self, user_id, nlikes=None, nfollows=None):
    self.console_print("Like followers of: %s." % user_id)
    if self.reached_limit('likes'):
        self.console_print("Out of likes for today.")
        return
    if not user_id:
    ...
    if not follower_ids:
        self.console_print("%s not found / closed / has no followers." % user_id)
    else:
        self.like_users(follower_ids[:nfollows], nlikes)


def like_following(self, user_id, nlikes=None, nfollows=None):
    self.console_print("Like following of: %s." % user_id)
    if self.reached_limit('likes'):
        self.console_print("Out of likes for today.")
        return
    if not user_id:
    ...
    if not following_ids:
        self.console_print("%s not found / closed / has no following." % user_id)
    else:
