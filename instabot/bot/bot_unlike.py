...
def unlike(self, media_id):
    if not self.reached_limit('unlikes'):
        ...
        self.console_print("Out of unlikes for today.")
    return False
...
def unlike_medias(self, medias):
    broken_items = []
    self.console_print("Going to unlike %d medias." % (len(medias)))
    for media in tqdm(medias):
        if not self.unlike(media):
            self.error_delay()
            broken_items = medias[medias.index(media):]
            break
    self.console_print("DONE: Total unliked %d medias." % self.total['unlikes'])
    return broken_items


def unlike_user(self, user_id):
    self.console_print("Going to unlike user %s's feed:" % user_id)
