...
def block(self, user_id):
    ...
    if not self.reached_limit('blocks'):
        ...
        self.console_print("Out of blocks for today.")
    return False
...
def block_users(self, user_ids):
    broken_items = []
    self.console_print("Going to block %d users." % len(user_ids))
    for user_id in tqdm(user_ids):
        if not self.block(user_id):
            self.error_delay()
            broken_items = user_ids[user_ids.index(user_id):]
            break
    self.console_print("DONE: Total blocked %d users." % self.total['blocks'])
    return broken_items


def unblock_users(self, user_ids):
    broken_items = []
    self.console_print("Going to unblock %d users." % len(user_ids))
    for user_id in tqdm(user_ids):
        if not self.unblock(user_id):
            self.error_delay()
            broken_items.append(user_id)
    self.console_print("DONE: Total unblocked %d users." % self.total['unblocks'])
    return broken_items


def block_bots(self):
    self.console_print("Going to block bots.")
