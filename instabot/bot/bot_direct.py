...
def send_message(self, text, user_ids, thread_id=None):
    ...
    if self.reached_limit('messages'):
        self.console_print("Out of messages for today.")
        return False
    ...
    self.console_print("Message to {user_ids} wasn't sent".format(user_ids=user_ids))
    return False
...
def send_messages(self, text, user_ids):
    broken_items = []
    if not user_ids:
        self.console_print("User must be at least one.")
        return broken_items
    self.console_print("Going to send %d messages." % (len(user_ids)))
